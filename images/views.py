...
def image_create(request):
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            messages.success(request, 'Immage added successfully')
...
def image_detail(request, id, slug):
    ...
    r.incrby('image_ranking', 1, image.id)
    return render(request, 'image/detail.html', {'section': 'images', 'image': image, 'total_views': total_views})
...
def image_ranking(request):
    ...
    most_viewed.sort(key=lambda x: image_ranking_ids(x.id))
